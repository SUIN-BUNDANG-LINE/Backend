...
- 시나리오 ↔ Kotlin 테스트 매핑: k6_scripts/README.md
...
### 5. outbox-producer-dlq

| 항목 | 내용 |
|---|---|
| 대상 대시보드 | `outbox-pipeline` (primary) |
| 핵심 패널 | FAILED by instance (0 → 1) · Publish Rate by status — failed 스파이크 5회 |
| 기대 결과 | 잘못된 토픽 PENDING 1건 → 5회 재시도 → `status=FAILED`, `retry_count=5` |
| 디버깅 PromQL | `outbox_events_failed` · `sum by (status) (increase(outbox_relay_publish_total{status="failed"}[5m]))` |

### 6. skip-locked-concurrency
...
| 항목 | 내용 |
|---|---|
| 대상 대시보드 | `sms-failover` (primary), `consumer-fanout` (DLT 토픽 보조) |
| 핵심 패널 | SMS Notification Jobs by Status · FAILED Alert · DLT Messages by Type · DLT Consumer Lag · SMS Attempts Rate by Result |
| 기대 결과 | 추첨 1건 100% 당첨 → 90초 내 `jobs.status=FAILED` + `retry_count=5` + dlt_messages 1건 |
| 디버깅 PromQL | `sum by (status) (sms_notification_jobs_count)` · `sum by (notification_type) (dlt_messages_total)` · `sum by (result) (rate(sms_notification_retry_total[1m]))` |

### 8. survey-response-load (후속 계획)

| 항목 | 내용 |
|---|---|
| 상태 | k6 스크립트 미작성 (T7에서 누락). 후속 태스크에서 추가 예정 |
| 의도 | 설문 응답 부하 → outbox → kafka → response-stats consumer fan-out 검증 |
| 대상 대시보드 | `consumer-fanout` — Topic: survey-response-submitted 패널 |
