...
/**
 * 무보호 기준선 (비교 실험 전용) — 경쟁 제어를 전혀 하지 않는다.
 *
 * 경쟁 제어 지점: **없음**. 트랜잭션 하나만 열고 곧바로 보드를 읽고 쓴다.
 * 동시 요청은 InnoDB 행 락에 맡겨지며, 그 대가(데드락 롤백)를 그대로 드러내는 것이 이 경로의 목적이다.
 */
@Component
class NoLockDrawingStrategy(
    ...
    companion object {
        private val log = LoggerFactory.getLogger(NoLockDrawingStrategy::class.java)
    }
