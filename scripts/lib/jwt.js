...
//   3) application-secret.yml의 jwt.secret-key (k6를 scripts/에서 실행할 때 자동 로드)
//
...
// k6 open()은 호출 파일(jwt.js) 위치 기준 상대경로 — scripts/lib/ → repo root는 2단계 위
    ...
    if (!secret) {
        throw new Error(
            'JWT secret을 찾지 못했습니다. JWT_SECRET_KEY ENV를 주거나, ' +
                'k6를 scripts/ 디렉토리에서 실행하세요(application-secret.yml 자동 로드).',
        );
    }
